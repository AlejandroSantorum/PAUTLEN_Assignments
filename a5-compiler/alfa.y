...
%type <attr_type> condicional
%type <attr_type> comparacion
%type <attr_type> elemento_vector
%type <attr_type> exp
%type <attr_type> constante
%type <attr_type> constante_entera
%type <attr_type> constante_logica
%type <attr_type> identificador
...
programa:
    TOK_MAIN TOK_LLAVEIZQUIERDA declaraciones init_sbtb_data funciones write_main init_main sentencias TOK_LLAVEDERECHA { fprintf(yyout, ";R1:\t<programa> ::= main { <declaraciones> <funciones> <sentencias> }\n");}
;
...
clase_vector:
    TOK_ARRAY tipo TOK_CORCHETEIZQUIERDO constante_entera TOK_CORCHETEDERECHO { fprintf(yyout, ";R15:\t<clase_vector> ::= array <tipo> [ <constante_entera> ]\n"); }
;
...
bucle:
    TOK_WHILE TOK_PARENTESISIZQUIERDO exp TOK_PARENTESISDERECHO TOK_LLAVEIZQUIERDA sentencias TOK_LLAVEDERECHA { fprintf(yyout, ";R52:\t<bucle> ::= while ( <exp> ) { <sentencias> }\n"); }
;

lectura:
    TOK_SCANF identificador { fprintf(yyout, ";R54:\t<lectura> ::= scanf <identificador>\n"); }
;
...
comparacion:
    exp TOK_IGUAL exp { fprintf(yyout, ";R93:\t<comparacion> ::= <exp> == <exp>\n"); }
...
constante_logica:
    TOK_TRUE { fprintf(yyout, ";R102:\t<constante_logica> ::= true\n"); }
